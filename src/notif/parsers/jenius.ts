...
export const jenius: ParserFn = (n) => {
  const hay = `${n.title || ''} ${n.text}`;
  if (!/Jenius|BTPN/i.test(hay) && !(n.app && /jenius|btpn/i.test(n.app))) return null;
  ...
  const merchantMatch =
    ...
    text.match(/(?:ke|kepada|merchant|penerima|to)[:\s]+([A-Z0-9 .,&'/-]{3,60})/i);
